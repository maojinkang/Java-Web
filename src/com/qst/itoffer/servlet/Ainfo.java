...
public class Ainfo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		Achievementinfo aa = dao.select(id);
		if (type.equals("select")) {
			if (aa == null && class_id != 0) {
				out.print("<script type='text/javascript'>");
				out.print("alert('无成绩！！');");
				out.print("window.location='SelectStudentServlet?class_id=" + class_id + "';");
				out.print("</script>");
			} 
			if(aa==null && class_id==0) {
				out.print("<script type='text/javascript'>");
		...
		if (type.equals("save")) {
			if (aa != null) {
				...
				dao.save(info);
				response.sendRedirect("SelectStudentServlet?class_id=" + class_id);
			}
		}
